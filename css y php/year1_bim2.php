...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO year1_bim2 (bimestre, matricula_alumno, nombre_alumno, esp1, ingles1, matematicas1, ciencias1_biologia, geografia_mexico, educacion_fisica1, tecnologia1, artes1, asignatura_estatal, tutoria, tic, vida_saludable) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
$v1_alumno = "0";
if (isset($_POST['var1'])) {
  $v1_alumno = $_POST['var1'];
}
$v2_alumno = "0";
if (isset($_POST['var2'])) {
  $v2_alumno = $_POST['var2'];
}
mysql_select_db($database_proyecto, $proyecto);
$query_alumno = sprintf("SELECT matricula_alumno, nombre_alumno, apellido_paterno, id_grupo, grado, grupo FROM alumno where grado=%s and id_grupo=%s", GetSQLValueString($v1_alumno, "int"),GetSQLValueString($v2_alumno, "int"));
$alumno = mysql_query($query_alumno, $proyecto) or die(mysql_error());
...
<div  class="cabezal">
  ...
  <table width="454" border="1" align="center">
    <tr>
      <td>INSERCION DE CALIFICACIONES PROMER AÑO BIMESTRE 2</td>
    </tr>
  </table>
...
?>
  ...
  <input type="submit" name="button2" id="button2" value="buscar" />
    ...
    <table border="1">
      ...
      <?php do { ?>
        <tr>
          <td><?php echo $row_alumno['matricula_alumno']; ?></td>
          <td><?php echo $row_alumno['nombre_alumno']; ?></td>
          <td><?php echo $row_alumno['apellido_paterno']; ?></td>
          <td><?php echo $row_alumno['id_grupo']; ?></td>
          <td><?php echo $row_alumno['grado']; ?></td>
          <td><?php echo $row_alumno['grupo']; ?></td>
        </tr>
